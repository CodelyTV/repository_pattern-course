...
import { UserRegisteredDomainEvent } from "../../../users/domain/UserRegisteredDomainEvent";
	...
	implements DomainEventSubscriber<UserRegisteredDomainEvent>
{
	...
	async on(event: UserRegisteredDomainEvent): Promise<void> {
		await this.sender.send(event.name, event.email);
	}
	...
	name(): string {
		return "codely.mooc.send_welcome_email_on_user_registered";
	}
}
